...
import { generateFunnyImagePrompts, analyzeImageContext, editImageWithPrompt, generateFunnyImage, fileToB64 } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const handleMagicVariations = async () => {
    if (!currentImage) return;
    setStatus(AppStatus.GENERATING_PROMPTS);
    try {
      const suggestions = await generateFunnyImagePrompts(currentImage);
  ...
  const handleEditImage = async (promptOverride?: string) => {
    ...
    try {
      const newImageB64 = await editImageWithPrompt(currentImage, promptToUse);
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      const newImageB64 = await generateFunnyImage(generatePrompt);
      setCurrentImage(newImageB64);
      setGeneratePrompt('');
      resetState();
      setStatus(AppStatus.SUCCESS);
    ...
    <div className="min-h-screen bg-slate-900 text-white font-sans selection:bg-brand-500 selection:text-white overflow-hidden flex flex-col md:flex-row">
      <aside className="w-full md:w-80 bg-slate-800 border-r border-slate-700 flex flex-col overflow-y-auto h-1/3 md:h-screen shrink-0 z-20 shadow-xl">
        <div className="p-6">
          ...
          <div className="mb-8">
            ...
            <button onClick={() => fileInputRef.current?.click()} className="w-full py-3 px-4 bg-slate-700 hover:bg-slate-600 rounded-xl border border-slate-600 transition flex items-center justify-center gap-2 font-medium mb-4">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
              Upload Image
            </button>
            
            <div className="bg-slate-900/50 p-3 rounded-xl border border-slate-700">
              <h3 className="text-xs font-semibold text-slate-400 mb-2">Or Generate with AI</h3>
              <textarea 
                value={generatePrompt} 
                onChange={(e) => setGeneratePrompt(e.target.value)} 
                placeholder="E.g., A cat wearing sunglasses riding a skateboard" 
                className="w-full h-20 bg-slate-800 border border-slate-600 rounded-lg p-2 text-sm focus:ring-2 focus:ring-brand-500 outline-none mb-2 resize-none" 
              />
              <button 
                onClick={handleGenerateImage} 
                disabled={!generatePrompt.trim() || status !== AppStatus.IDLE && status !== AppStatus.SUCCESS} 
                className="w-full py-2 bg-brand-600 hover:bg-brand-500 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg text-white font-semibold transition text-sm flex items-center justify-center gap-2"
              >
        ...
        {status !== AppStatus.IDLE && status !== AppStatus.SUCCESS && (
             <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-50 flex flex-col items-center justify-center text-center p-6">
                ...
                <p className="text-xl font-semibold animate-pulse">
                    ...
                    {status === AppStatus.GENERATING_PROMPTS && "Thinking of funny variations..."}
                    {status === AppStatus.EDITING_IMAGE && "Working magic on the image..."}
                    {status === AppStatus.GENERATING_IMAGE && "Generating your masterpiece..."}
                </p>
             </div>
        )}
      </main>
      ...
      <aside className="w-full md:w-96 bg-slate-900 border-l border-slate-800 flex flex-col h-1/3 md:h-screen shrink-0 z-20">
        <div className="flex border-b border-slate-800">
            {[
              { id: 'caption', label: 'Variations', color: 'brand' },
            ...
            {activeTab === 'caption' && (
                <div className="space-y-6">
                     <div className="bg-slate-800/50 p-4 rounded-xl border border-brand-500/20">
                        ...
                        <p className="text-slate-400 text-xs mb-4">Let Gemini suggest funny ways to edit this image.</p>
                        <button onClick={handleMagicVariations} disabled={!currentImage} className="w-full py-2 bg-brand-600 hover:bg-brand-500 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg text-white font-semibold transition">
                            Generate Variations
                        </button>
                     </div>
                     {suggestedPrompts.length > 0 && (
                        <div className="space-y-2">
                            ...
                            {suggestedPrompts.map((prompt, idx) => (
                                <button key={idx} onClick={() => { setEditPrompt(prompt); handleEditImage(prompt); }} className="w-full text-left p-3 bg-slate-800 hover:bg-slate-700 rounded-lg border border-slate-700 hover:border-brand-500/50 transition text-sm group">
                                    <span className="text-slate-300 group-hover:text-white">{prompt}</span>
                                </button>
                            ))}
                        </div>
                     )}
